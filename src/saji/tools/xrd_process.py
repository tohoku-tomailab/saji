...
TOOL = Tool(
    ...
    params=[
        Param("bin", int, None, min=1, group="前処理", label="ビニング点数", help="N 点ずつ平均してビニングする（例 25。未指定ならしない）"),
        Param("bg", choices=("none", "arpls", "snip"), default="arpls", group="背景除去",
              label="背景除去", help="背景除去の方法"),
        Param("lam", float, 1e6, min=0, group="背景除去", advanced=True, label="arPLS λ", help="arPLS の滑らかさ"),
        Param("snip_window", int, 120, min=1, unit="点", group="背景除去", advanced=True,
              label="SNIP 最大半窓", help="SNIP の最大半窓"),
        Param("snip_smooth", int, 3, min=0, unit="点", group="背景除去", advanced=True,
              label="SNIP 平滑半窓", help="SNIP の平滑半窓"),
        Param("smooth", choices=("none", "savgol", "moving_average", "gaussian"), default="none",
              group="平滑化", label="平滑化", help="平滑化の方法"),
        Param("sg_window", int, 11, min=3, unit="点", group="平滑化", advanced=True,
              label="SG 窓", help="Savitzky-Golay の窓（偶数なら奇数に直す）"),
        Param("sg_poly", int, 3, min=0, group="平滑化", advanced=True, label="SG 次数", help="Savitzky-Golay の多項式次数"),
        Param("ma_window", int, 9, min=1, unit="点", group="平滑化", advanced=True, label="移動平均 窓", help="移動平均の窓"),
        Param("gauss_sigma", float, 2.0, min=0, unit="点", group="平滑化", advanced=True,
              label="ガウシアン σ", help="ガウシアンの σ"),
        Param("norm", choices=("none", "max", "area", "reference"), default="none", group="規格化",
              label="規格化", help="規格化の方法（max=最大ピーク / area=積分強度 / reference=指定ピーク）"),
        Param("norm_target", float, 1.0, group="規格化", label="目標値", help="規格化後の目標値（100 で % 表示）"),
        Param("norm_ref_pos", float, None, unit="deg", group="規格化",
              label="基準ピーク位置", help="reference 用: 基準ピークの 2θ（例 Cu(111) なら 43.3）"),
        Param("norm_ref_tol", float, 0.3, min=0, unit="deg", group="規格化", advanced=True,
              label="基準ピーク探索幅", help="reference 用: 基準ピークを探す幅（±）"),
        ...
        Param("show_raw", bool, True, group="図", label="生データも描く", help="生データを重ねて描く"),
        STYLE_PARAM,
    ],
